...
The first two will use the scenario file referred to by the https://publications.pages.asam.net/standards/ASAM_OpenSCENARIO/ASAM_OpenSCENARIO_XML/latest/generated/content/ParameterValueDistribution.html[ParameterValueDistriution element] inside the parameter value distribution file (`cut-in_parameter_set.xosc`). The third example will apply the parameter settings on the scenario defined by the `osc` launch argument, overriding any scenario reference in the specified `param_dist` file. This can be useful when a single parameter distribution applies to multiple scenarios (instead of having one parameter file for each scenario).
...
Most vehicle models have been updated with simple polygon faces representing each possible light source. Note that only the light faces themselves are illuminated, no light are casted on the surrounding environment.

By default no light faces are shown, only if the
...
esmini supports limited previeweing of light state actions. Most vehicle models
...
2. By API function SE_SetOption(), see <<Program Options>>
...
== About this document
The https://github.com/esmini/esmini/blob/master/docs/user_guide.adoc[source] of this document is written in the https://asciidoc-py.github.io/index.html[asciidoc] format.

=== How to generate HTML from AsciiDoc
The html is generated with https://asciidoctor.org/[asciidoctor]. Code snippets, images and other resources are included in the process. Also the version numbers are automatically inserted to avoid "human errors".
...
Generate html: ``asciidoctor -r asciidoctor-kroki user_guide.adoc -o index.html``

=== How to view previous versions
Full commit history is found https://github.com/esmini/esmini.github.io/commits/main[here]

To view a previous version, utilizing https://htmlpreview.github.io/[htmlpreview.github.io], either refer to a https://github.com/esmini/esmini.github.io/tags[tag], like this:
